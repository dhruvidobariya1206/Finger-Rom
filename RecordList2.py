...
class RecordList2(tk.Tk):

    def __init__(self, Patient_id, hand, *args, **kwargs):
        ...
        self.title("List Records")

        text_styles = {"font": ("Verdana", 10),
                       "background": "#8D99AE",
                       "foreground": "#8D99AE"}

        canvas = tk.Canvas(main_frame, width=200, height=150, bg="#9EAABF")
        ...
        options = ['C', 'J']
        clicked = StringVar()
        clicked.set(options[0] if options[0]==Patient_id[0] else options[1])
        drop = OptionMenu(main_frame, clicked, *options)
        drop.place(x=110, y=59)

        en1 = Entry(main_frame, width=10, text=Patient_id[1], validate="key",
        ...
        if (row != None):
            ...
            if (len(pip_details) == 0 or len(dip_details) == 0 or len(mcp_details) == 0):
                ...
                line1.get_tk_widget().place(x=120,y=pos+50)
                for i in range(1,5):
                    dip[i].plot(kind='line', label=f"{lbl[i-0]}", legend=True, ax=graph_tip, marker="o")
                ...
                line1.get_tk_widget().place(x=470,y=pos+50)
                for i in range(1,5):
                    pip[i].plot(kind='line', label=f"{lbl[i-1]}", legend=True, ax=graph_pip, marker="o")
                ...
                line1.get_tk_widget().place(x=820,y=pos+50)
                for i in range(1,5):
                    mcp[i].plot(kind='line', label=f"{lbl[i-1]}", legend=True, ax=graph_mcp, marker="o")
        ...
        def view():
            ...
            if (str(en1.get()) == ""):
                ...
                Patient_id_new = clicked.get()+str(en1.get())
                
                hand_new = hand_value.get()
                # print(Patient_id)
                RecordList2.destroy(self)
                RecordList.RecordList(Patient_id_new, hand_new)
